...
template <auto raw_data>
    requires(std::same_as<std::remove_cvref_t<decltype(raw_data)>,
                          detail::huffman_string_container<
                              std::remove_cvref_t<decltype(raw_data.data[0])>,
                              raw_data.size()>>
             && raw_data.size() > 0)
...
private:
    ...
    /**
     * Builds a list of nodes for every character that appears in the given
     * data. This list is sorted by increasing frequency.
    ...
    consteval static auto build_node_list() noexcept {
        ...
        // Filter out the non-occuring values, and build a compact list to
        // return
        auto first_valid_node
            = std::find_if(list.begin(), list.end(), [](const auto &n) {
                  return n.freq != 0;
              });
    ...
    consteval static auto build_node_tree() noexcept {
        ...
        while (true) {
            ...
            node new_node{next_parent_node_value++,
            ...
            *--tree_begin = list[1];
            ...
            auto insertion_point
                = std::find_if(list.begin(),
                               list_end - 1,
                               [&new_node](const auto &n) {
                                   return n.freq >= new_node.freq;
                               });
        ...
        for (auto iter = tree.begin(); ++iter != tree.end();) {
            if (iter->parent == -1) {
                auto parent
                    = std::find_if(tree.begin(), iter, [&iter](const auto &n) {
                          return n.left == iter->value
                                 || n.right == iter->value;
                      });
                if (parent != iter) {
    ...
    consteval static auto compressed_size_info() noexcept {
        ...
        for (usize_t i = 0; i < raw_data.size(); i++) {
            ...
            auto leaf
                = std::find_if(tree.begin(), tree.end(), [c](const auto &n) {
                      return n.value == c;
                  });
    ...
    consteval void compress() noexcept {
        ...
        // Set up byte and bit count (note, we're compressing the data
        // backwards)
        ...
        // Compress data backwards, because we obtain the Huffman codes
        // backwards as we traverse towards the parent node.
        for (auto i = raw_data.size(); i > 0; i--) {
            auto c = static_cast<int>(raw_data[i - 1]);
            auto leaf = std::find_if(tree.begin(), tree.end(), [c](auto &n) {
                return n.value == c;
            });
...
public:
    ...
    public:
        ...
        decoder(const unsigned char *comp_data) noexcept
            : m_data(comp_data)
            , m_table(comp_data + compressed_size_info().first) {
            get_next();
        }
        decoder() = default;
...
private:
    ...
    unsigned char
        compressed_data[bytes_saved() > 0
                            ? compressed_size_info().first + 3 * tree_count()
                            : raw_data.size()]
        = {0};
};
...
template <typename T, T... list>
constexpr auto huffman_compress_array
    = detail::huffman_compress_array_container<T, list...>::data;
...
auto main() -> int {
    ...
    // auto data = "A long string to be compressed at compiled time"_huffman;
    auto data
